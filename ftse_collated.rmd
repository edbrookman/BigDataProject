...
FTAS_s <- FTAS_data %>%   select(ref_date, price_open, price_close, price_high, price_low, price_adjusted, volume,ret_adjusted_prices)
...
external_indicators <- external_indicators %>% select(-c(uk_carbon_price, `7day_elec_avg`, daily_elec_avg))

FTAS_s_combined <- merge(FTAS_s, external_indicators, by = "date", all.x = TRUE)
glimpse(FTAS_s_combined)

BVSP_s_combined <- merge(BVSP_s, external_indicators, by = "date", all.x = TRUE)
glimpse(BVSP_s_combined)

FTAS_s <- FTAS_s %>% select(-c(direction.1, direction))
BVSP_s <- BVSP_s %>% select(-c(direction.1, direction))
...
FTAS_s_combined <- FTAS_s_combined %>% select(-c(direction.1, direction, nz_carbon_price, kor_carbon_price))
...
BVSP_s <- BVSP_s %>% select(-c(`stoch_k`, `stoch_d`))
BVSP_s_combined <- BVSP_s_combined %>% select(-c(`stoch_k`, `stoch_d`))
...
BVSP_s_combined <- BVSP_s_combined %>% select(-c(direction.1, direction, nz_carbon_price, kor_carbon_price))
